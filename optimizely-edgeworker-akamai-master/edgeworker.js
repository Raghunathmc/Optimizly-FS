...
const ACTIVE_EXPERIMENTS = "live_chat";
...
async function getDecisionsForRequest(
  ...
  if (arrayIsValid(flagsToDecide) && ACTIVE_EXPERIMENTS) {
	...
	logger.log("getDecisionsForRequest-->userId-->",userId);
    if (attributes) {
    ...
    try {	
		for(var vs in decisions){
				logger.log("getDecisionsForRequest-->decisionsVal-->",vs);
				logger.log("getDecisionsForRequest-->decisionsVal-->",decisions[vs]);
		}		
...
async function onClientRequest(request) {
 ...
 logger.log("innnnnnnnnnnnnnnnnn onClientRequest");
